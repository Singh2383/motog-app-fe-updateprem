...
import FiltersSidebar from './_components/filters';
import CarsListing from './_components/car-listing';

const RESULTS_PER_PAGE = 8;

export default function UsedCarsPage({ params }: { params: { city: string } }) {
    ...
    const [priceRange, setPriceRange] = useState<[number, number]>([
        Number(sp.get('minPrice')) || 3,
        Number(sp.get('maxPrice')) || 5,
    ]);
    ...
    const queryParams = useMemo(() => ({
        ...
        city: params.city,           // optional extra param
    }), [brand, fuel, year, transmission, owner, priceRange, verifiedOnly, page, params.city]);
        ...
        <div className="bg-gray-50 min-h-screen pt-32 px-4 sm:px-6 lg:px-8">
            <header className="mb-6">
                <h1 className="text-2xl font-bold">
                    Used Cars in {(params?.city ?? 'your city').replace(/-/g, ' ')} (₹3 – 5 Lakh)
                </h1>
